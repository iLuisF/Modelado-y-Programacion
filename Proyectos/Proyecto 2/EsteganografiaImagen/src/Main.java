...
public class Main {
    ...
    public static void main(String[] args) {
        try {
            Esteganografia imagen = new Esteganografia(new Imagen(
                    args[0].equals("h")?(".\\" + args[2]):(".\\" + args[1])));
            ManejadorArchivo texto;
            switch (args[0]) {
                case "h":
                    texto = new ManejadorArchivo(args[1]);
                    imagen.codificarImagen(texto.leerArchivo(), args[3]);
                    break;
                case "u":                    
                    texto = new ManejadorArchivo(args[2]);
                    texto.escribirArchivo(imagen.decodificarImagen());
                    break;
                default:
