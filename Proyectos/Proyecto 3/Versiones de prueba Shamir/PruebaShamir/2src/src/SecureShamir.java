...
public class SecureShamir {
    ...
    public SecureShamir(BigInteger[] valor, int k, int n) {
        ...
        this.x = new ArrayList<>(valor.length);
     ...
     * Lagrange.
     */
    public BigInteger reconstruirSecreto(BigInteger[] valoresCompartidos) {
        BigInteger[] x = this.x.get(this.shares.indexOf(valoresCompartidos));
        ...
        for (int i = 0; i < k; i++) {
            ...
            secreto = secreto.add(valoresCompartidos[i].multiply(prod));
        }
        return secreto.mod(this.primoGrande);
    }
