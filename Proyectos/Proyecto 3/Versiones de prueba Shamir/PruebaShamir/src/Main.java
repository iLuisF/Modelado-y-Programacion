    ...
    public static void main(String[] args) {
        ...
        switch (opcion) {
            case "c":
                ...
                kEvaluaciones = "";
                kEvaluaciones = leer.nextLine();
                //String archivoCifrar = leer.nextLine(); pendiente                
                System.out.println("Escribe la contraseña");
                String contrasenia = leer.nextLine();
                llaveSimetrica = new SecureKeyGenerator(contrasenia);
                ManejadorArchivo archivo = new ManejadorArchivo(nArchivo);
                BigInteger llaveSim = llaveSimetrica.makeKey();
                Puntos secretoEncriptado = shamir.construirSecreto(
                        llaveSim, Integer.parseInt(kEvaluaciones),
                        Integer.parseInt(nEvalaluaciones)); //3 5
                archivo.escribirArchivo(secretoEncriptado.toSting());
                //Encriptar con shaes
                System.out.println("Nombre del archivo con el documento claro");
                String archivoClaro = leer.nextLine();
                File inputFile = new File(archivoClaro);
                File archivoEncriptado = new File(archivoClaro + ".aes");
                try{
                    System.out.println(String.valueOf(llaveSim));
                    Criptograma criptograma = new Criptograma();
                    String pruebaLlave = "EstoEsUnaContraseniaaswerfgtolkn";
                    criptograma.encriptar(String.valueOf(llaveSim), inputFile, archivoEncriptado);
            ...
            Logger.getLogger(Main.class.getName()).log(Level.SEVERE, null, ex);
                ...
                break;
            case "d":                
                System.out.println("Escribe el nombre del archivo con, al"
                        + "menos, k de las n evaluaciones del polinomio. "
                        + "Solo el nombre sin extención.");
                kEvaluaciones = "";
                kEvaluaciones = leer.nextLine() + ".frg";
                ManejadorArchivo leerArchivo = new ManejadorArchivo(kEvaluaciones);                
                Puntos puntos = new Puntos();
                puntos.convertirAPunto(leerArchivo.leerArchivo());                
                System.out.println(shamir.reconstruirSecreto(puntos.getPuntosX(), puntos.getPuntosY(), 3));                                    
                break;                           
            default:
